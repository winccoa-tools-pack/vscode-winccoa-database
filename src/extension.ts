...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewPanelSerializer('winccoa-database.configEditor', {
            async deserializeWebviewPanel(webviewPanel: vscode.WebviewPanel, state: unknown) {
                log.info(
                    `Deserializing config editor webview panel, state=${JSON.stringify(state)}`,
                );
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('winccoa-database.openConfigEditor', (item) => {
            log.info(
                `Command: openConfigEditor, item=${JSON.stringify(item?.label)}, dpId=${item?.dpId}, elId=${item?.elId}`,
            );
        ...
        vscode.commands.registerCommand('winccoa-database.deleteDpType', async (item) => {
            ...
            const confirmLabel =
                dpCount === 0 ? 'Delete' : `Delete Type and ${dpCount} Datapoint(s)`;
        ...
        vscode.commands.registerCommand(
            'winccoa-database.deleteDp',
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            async (item, selectedItems?: any[]) => {
                log.info(
                    `Command: deleteDp, item=${item?.label}, selectedCount=${selectedItems?.length ?? 1}`,
                );
                if (!item?.label) return;
...
async function initProjectConnection(context: vscode.ExtensionContext): Promise<void> {
    ...
    let configPath = vscode.workspace
        .getConfiguration('winccoa-database')
    ...
    if (api) {
        ...
        if (api.getCurrentProject) {
            ...
            if (project?.projectDir) {
                ...
                log.info(
                    'project-admin has no current project selected yet (will connect when user selects one)',
                );
            }
        }
    }
    ...
    log.info(
        `Step 3 - Workspace folders: ${workspaceFolders?.map((f) => f.uri.fsPath).join(', ') || '(none)'}`,
    );
    if (workspaceFolders) {
...
async function connectToProject(projectPath: string, version?: string): Promise<void> {
    ...
    log.info(
        `  last_value.sqlite exists: ${fs.existsSync(path.join(sqliteDir, 'last_value.sqlite'))}`,
    );
    ...
    try {
        ...
        if (mcpConfigured) {
            mcpClient.checkHealth().then((healthy) => {
                if (healthy) {
                    ...
                    log.warn(
                        'MCP HTTP server configured but not reachable - value setting will be unavailable',
                    );
                }
            });
        }
        ...
        vscode.window.showInformationMessage(
            `WinCC OA Database: Connected to ${path.basename(projectPath)} (${dpTypes.length} DPTs, ${datapoints.length} DPs)`,
        );
    } catch (err) {
...
async function selectProject(): Promise<void> {
    ...
    if (result && result.length > 0) {
        ...
        await vscode.workspace
            .getConfiguration('winccoa-database')
            .update('projectPath', projectPath, vscode.ConfigurationTarget.Global);
    } else {
